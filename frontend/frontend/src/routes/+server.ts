...
const coll = db.collection("repositories");
const collections = ["repositories", "metrics", "bak_metrics", "task_results"];
...
export async function POST() {
    ...
    `);
        ...
        let all = await query.all();
        // for await (const metric of all) {
        //     console.log(metric.task_id);
        // }
        // console.log(await coll.figures());
        // all["stats"]= await coll.figures();
        // console.log(stats);
